...
<%
	//if user type is 1 (admin), redirect user to admin dashboard
...
<body>
	...
	<div class="notify">
		<p>
			<%
				if (null != request.getAttribute("paid")) {
				out.println(request.getAttribute("paid"));
			}
			%>
		</p>
	</div>
					...
					<div class="search-event">
						<form action="">
							<input type="text" name="tag"
								placeholder="music/seminar/party/education"> <input
								type="submit" name="submit" value="Search">
						</form>
					</div>

					<!-----------------------if user searched for an event section---------------------->

					<%
						String x = request.getParameter("submit");
						String keyword = request.getParameter("tag");
						if (x != null && x.equals("Search")) {
					%>
							...
							<table class="table table-hover">
								...
								<tbody>
									...
									<%
										...
										ResultSet rs = stmt
										.executeQuery("select * from event where status = 1 AND tags LIKE '%" + keyword + "%' ORDER BY eId DESC ");
										/*if( rs.next() == false){
									...
									<tr>
										...
										<br> <a href="event.jsp?eId=<%=rs.getString("eId")%>"
											class="btn btn-primary">view</a></td>
										<td><%=rs.getString("summary")%></td>
										<td><%=rs.getString("venue")%> <br>
										<br> <%=rs.getString("date")%> <br> <%
							...
							<table class="table table-hover">
								...
								<tbody>
									...
									<%
										...
										Connection connection = connectionUtil.getConnection();
										Statement stmt = connection.createStatement();
										ResultSet rs = stmt.executeQuery("select * from event where status = 1 ORDER BY eId DESC ");
										/*if( rs.next() == false){
											out.print("<p>No results found</p>");
										}*/
										while (rs.next()) {
